...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    if word_1 != candidate_ladder[0] or word_2 != candidate_ladder[-1] or len(word_2) != len(word_1):
        return False
    ...
    for i in range(len(word_2)):
        if word_1[i] != word_2[i]:
            diff += 1
    step = 0
    for i in range(len(candidate_ladder)):
        if candidate_ladder[i] not in lexicon:
            return False
        if i < len(candidate_ladder)-1:
            diff_single = 0
            for j in range(len(word_1)):
                if candidate_ladder[i][j] != candidate_ladder[i+1][j]:
                    diff_single += 1
            if diff_single != 1:
                return False
            step += 1
